...
class storage(object):
    ...
    def _experimental(self):
        cmd = 'SELECT * FROM node_004 ORDER BY Timestamp DESC LIMIT 5'
        tmp = self.c.execute(cmd)
        tmp = [list(r) for r in zip(*tmp)]
        print tmp
...
if '__main__' == __name__:
    ...
    tmp = store.read_latest(4)
    print
    print tmp

    # hourly average of the past 1 day, 3 hours
    tmp = store.read(4,nhour=3,nday=1,avg='hourly')
    print
    print tmp.keys()
    print len(tmp['Temp_BMP180'])

    # daily average of the past 14 days
    tmp = store.read(4,nday=14,avg='daily')
    print
    print tmp.keys()
    print len(tmp['Temp_BMP180'])

    # past 6 hours of data
    tmp = store.read(4,nhour=6)
    print
    print tmp.keys()
    print len(tmp['Temp_BMP180'])
